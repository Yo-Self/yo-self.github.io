-- YOS-27: rate-limit delivery geocoding abuse; restrict geocode_delivery_address to SECURITY DEFINER callers.

CREATE OR REPLACE FUNCTION public.assert_delivery_order_rate_limit(
...
BEGIN
  ...
  WHERE e.restaurant_id = p_restaurant_id
    AND e.scope_key = 'delivery:restaurant'
    AND e.created_at > now() - interval '5 minutes';

  IF v_restaurant_count >= 15 THEN
    RAISE EXCEPTION 'delivery_rate_limited';
  END IF;

  IF p_address_hash IS NOT NULL AND length(p_address_hash) > 0 THEN
    SELECT count(*)::integer
    INTO v_address_count
    FROM public.customer_order_rate_events e
    WHERE e.restaurant_id = p_restaurant_id
      AND e.scope_key = 'delivery:address:' || p_address_hash
...
CREATE OR REPLACE FUNCTION public.record_delivery_order_rate_event(
  p_restaurant_id uuid,
  p_address_hash text
)
RETURNS void
...
BEGIN
  ...
  VALUES (p_restaurant_id, 'delivery:restaurant');

  IF p_address_hash IS NOT NULL AND length(p_address_hash) > 0 THEN
    INSERT INTO public.customer_order_rate_events (restaurant_id, scope_key)
    VALUES (p_restaurant_id, 'delivery:address:' || p_address_hash);
  END IF;
END;
$$;
...
BEGIN
  ...
  IF v_order_type = 'delivery' THEN
    PERFORM public.record_delivery_order_rate_event(v_restaurant_id, v_address_hash);
  END IF;
