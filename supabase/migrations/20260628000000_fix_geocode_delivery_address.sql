-- Fix geocode_delivery_address: the http extension lives in public schema, not extensions.
-- The broken extensions.http_request cast caused every delivery checkout to fail with geocoding_failed.
...
BEGIN
  ...
  FOR v_attempt IN 1..2 LOOP
    BEGIN
      ...
      INTO v_response
      FROM public.http_get(v_url) AS h;
...
-- When geocoding is unavailable, trust Google Places coordinates already validated client-side.
...
BEGIN
  ...
  IF v_order_type = 'delivery' THEN
    ...
    v_fee_lat := v_customer_lat;
    v_fee_lng := v_customer_lng;

    BEGIN
      SELECT g.lat, g.lng INTO v_geocoded_lat, v_geocoded_lng FROM public.geocode_delivery_address(v_delivery_address) AS g;
      ...
      END IF;
      v_fee_lat := v_geocoded_lat;
      v_fee_lng := v_geocoded_lng;
    EXCEPTION
      WHEN OTHERS THEN
        IF SQLERRM LIKE 'geocoding_%' THEN
          v_fee_lat := v_customer_lat;
          v_fee_lng := v_customer_lng;
        ELSE
          RAISE;
    ...
    SELECT d.fee_cents, d.distance_km, d.covered, d.reason INTO v_delivery FROM public.compute_delivery_fee_cents(v_restaurant_id, v_fee_lat, v_fee_lng) AS d;
